...
def collater(datas):
    ...
    out_data_dict1 = {}
    out_data_dict2 = {}
    for name in datas[0][0]:
        out_data_dict1[name] = []
    for name in datas[0][1]:
        out_data_dict2[name] = []
    for samples in datas:
        for name in samples[0]:
            out_data_dict1[name].append(torch.from_numpy(samples[0][name]))
        for name in samples[1]:
            out_data_dict2[name].append(torch.from_numpy(samples[1][name]))

    for name in out_data_dict1:
        out_data_dict1[name] = torch.stack(out_data_dict1[name], dim=0)
    for name in out_data_dict2:
        out_data_dict2[name] = torch.stack(out_data_dict2[name], dim=0)
    return [out_data_dict1,out_data_dict2]
...
class Network(object):
    ...
    def train_network(self, args):
        save_path = 'weights23_'+args.dataset
        ...
        for data_dict in data_loader:
            for i in range(2):
                for name in data_dict[i]:
                    data_dict[i][name] = data_dict[i][name].to(device=self.device)
            if phase == 'train':
                self.optimizer.zero_grad()
                with torch.enable_grad():
                    pr_decs = self.model([data_dict[0]['input'],data_dict[1]['input']]) 
