...
def processing_test(image, input_h, input_w):
    ...
    out_image = out_image.transpose(2, 0, 1).reshape(3, input_h, input_w)
...
def generate_ground_truth(image,
    ...
    seg_img_new = np.zeros((1, image_h, image_w))
    ...
    for k in range(17):
        pts = pts_2[4 * k:4 * k + 4, :]  # ndarray(4,2)
        ...
        pts_int[2] = pts_int[3]
        pts_int[3] = tmp
        cv2.fillPoly(seg_img_new[0, :, :], [pts_int], (255, 255, 255))
    seg_img_new[seg_img_new != 0] = 1
