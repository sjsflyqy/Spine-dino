...
def _hrnet(arch, pretrained, progress, **kwargs):
    # 当前统一工程只保留 HRNet18，因此配置和权重文件都只围绕这一种结构。
    from .hrnet18_config import MODEL_CONFIGS
    ...
    if pretrained:
        ...
        # HRNet 主干预训练权重已经迁入统一工程本地目录，不再从网络下载。
        pretrained_weights_path = base_path / "models" / "slippage_backbone_hrnet18_imagenet.pth"
        
        if not pretrained_weights_path.exists():
            raise FileNotFoundError(f"找不到 HRNet 预训练权重文件：{pretrained_weights_path}")
