...
# PyTorch 2.5 exposes the composable FSDP2 API from the private
# ``torch.distributed._composable`` namespace.  Newer PyTorch releases export
# the same symbols from ``torch.distributed.fsdp``.  Supporting both keeps the
# repository usable in the existing Spine-DINO environment without changing
# FSDP semantics.
try:
    from torch.distributed.fsdp import register_fsdp_forward_method
    from torch.distributed.fsdp._fully_shard._fsdp_state import FSDPState
except (ImportError, ModuleNotFoundError):  # PyTorch 2.5
