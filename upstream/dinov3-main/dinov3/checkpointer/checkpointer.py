...
def init_fsdp_model_from_checkpoint(
    ...
    if not Path(checkpoint_path).is_dir():  # PyTorch standard checkpoint
        ...
        # ``src_data_rank`` was added after PyTorch 2.5. Every rank reads the
        # same local initialization checkpoint here, so the 2.5 call without a
        # source rank has identical semantics for this use case.
        import inspect

        distribute_tensor = torch.distributed.tensor.distribute_tensor
        supports_src_data_rank = "src_data_rank" in inspect.signature(distribute_tensor).parameters

        def distribute(tensor):
            if supports_src_data_rank:
                return distribute_tensor(tensor, world_mesh, src_data_rank=None)
            return distribute_tensor(tensor, world_mesh)

        chkpt = {
            key: distribute(tensor)
