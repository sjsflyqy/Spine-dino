...
def free_if_fsdp(x):
    if is_sharded_fsdp(x):
        # PyTorch <= 2.0 exposed a list as ``_handles`` and accepted lists in
        # ``_reshard``.  PyTorch 2.5 stores one handle per FSDP instance as
        # ``_handle`` and changed ``_reshard`` to take a single handle/bool.
        # DINOv2 calls this after the teacher forward to release its unsharded
        # parameters early, so support both private API layouts.
        handles = getattr(x, "_handles", None)
        if handles is not None:
            _reshard(x, handles, [True for _ in handles])
            return
